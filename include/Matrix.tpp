...
namespace silent
{
  ...
  Matrix<T,Alloc>& Matrix<T,Alloc>::operator-=(const Matrix<T,Alloc>& other) {
    ...
    ::silent::aimpl::subtract(*this,other);
  ...
  Matrix<T,Alloc>& Matrix<T,Alloc>::operator*=(const Matrix<T,Alloc>& other) {
    ...
    ::silent::aimpl::multiply(*this, other);
    return *this;
  }

  /// Using a *= b
  template<typename T,class Alloc>
  Matrix<T,Alloc>& Matrix<T,Alloc>::operator*=(const T& other) {
  ...
  Matrix<T,Alloc> operator+(const Matrix<T,Alloc>& a,
    ...
    Matrix<T,Alloc> c(a.rows(),a.cols(),silent::DoNotInitialize);
    ::silent::aimpl::add(a,b,c);
    return c;
  }
  ...
  // Still not optimized with simd due to order in the files
  template<typename T,class Alloc>
  std::vector<T> operator*(const Matrix<T,Alloc>& a,
      ...
      if((a.cols() != b.size())){
        ...
        throw silent::Exception("Sizes don't match");
      }
      std::vector<T> v(a.rows(),T(0)); //Result vector
      for (size_t i = 0; i < a.rows(); ++i){ //For each column
        for(size_t j = 0; j < b.size(); ++j){//For each row/element in b
            v[i]+= a(i, j) * b[j];
        }
      }
      return v;
