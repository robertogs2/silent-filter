...
#ifndef SILENT_MATRIX_HPP
#define SILENT_MATRIX_HPP
...
namespace silent
{
   ...
   * use silent::aligned_allocator and for forcing the alignment of each
   * row you can use silent::aligned_row_allocator, both defined in
   * <Allocator.hpp>.
   */
  template<typename T,class Alloc=silent::aligned_row_allocator<T> >
  class Matrix {
  public:   
    /**
     * @name Standard types
  ...
  public:
     ...
     *
     * \code
     * silent::Matrix<int> m={ {1,2,3}, {4,5,6} };
     * \endcode
     */
    ...
    /// Multiply with another matrix and leave result here
    Matrix& operator*=(const Matrix& other);

    /// Multiply with a scalar and leave result here
